...
class Allpost extends CI_Controller{
    ...
    public function index(){
        ...
        $department = $this->session->userdata('admin_logged_in')['department'];
        $data['course_offered_bysemester'] = $this->Post_model->post_list($department,$admissioned_semester,$present_semester);
    ...
    public function enroll_course(){
        ...
        if($result==null){
            $this->session->set_flashdata('massage','<div class="alert alert-danger" role="alert">Course has no class time on routine</div>');
        ...
        if($num<$limited_seat){
                    ...
                    $student_id = $this->input->post('student_id');
                    if ($this->form_validation->run() == FALSE){
                        if($this->pre_completed_check() == TRUE){
                            ...
                            $this->session->set_flashdata('massage','<div class="alert alert-danger" role="alert">You have Completed this course once. if you Want to add this course again contact with your advisor or you can send a mail to your advisor<br><a class="btn btn-primary " href="<?= base_url() ?>youtube.com">Mail</a></div>');
                                redirect('mydonecourses','refresh');
                        }
                }else{
    ...
    public function pre_completed_check(){
        $query = $this->db->query('SELECT * FROM completed_courses WHERE student_id="'.$this->input->post('student_id').'" and course_code="'.$this->input->post('course_code').'"')->result_array();
        if($query==null){
            return true;
        }else{
            return false;
